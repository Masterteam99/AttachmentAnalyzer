...
class GamificationService {
  ...
  async calculateUserLevel(userId: string): Promise<{
    ...
    try {
      ...
      // XP from workouts
      totalXP += userStats?.totalWorkouts * 10 || 0;
  ...
  async getMotivationalMessage(userId: string): Promise<string> {
    try {
      ...
      if (userStats?.currentStreak >= 7) {
        messages.push(`Amazing! You're on a ${userStats.currentStreak}-day streak! 🔥`);
      } else if (userStats?.currentStreak >= 3) {
        messages.push(`Great momentum! ${userStats.currentStreak} days and counting! 💪`);
      }
      
      // Progress-based messages
      if (userStats?.weeklyProgress >= userStats?.weeklyGoal) {
        messages.push("You've smashed your weekly goal! Time to celebrate! 🎉");
      } else if (userStats?.weeklyProgress === userStats?.weeklyGoal - 1) {
      ...
      if (recentAchievements.length > 0) {
        ...
        const timeSince = Date.now() - new Date(latestAchievement.earnedAt).getTime();
